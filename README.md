...
##### Removed
Optional field `yearOfMigration` has been removed from the response body for the following endpoint:
...
- Add a new enum value `partner-income` to `inputs.incomeSources.nonBusinessIncomeSources[].incomeSourceType` and `calculation.endOfYearEstimate.incomeSource[].incomeSourceType`
...
New API version 1.0 added in Production with the following changes:
...
- Remove the `RULE_TAX_YEAR_NOT_ENDED` error to allow in-year submissions
...
Deprecate the following optional fields from the response body for all tax years
...
Version 2.0 updated in Sandbox with the following changes:
...
Add documentation response body example reflecting the aligned due date changes for tax years 2026–27 onwards for the following endpoint:

- Retrieve Income Tax (Self Assessment) Income and Expenditure Obligations

##### Changed

Update Sandbox due dates for `CUMULATIVE` and `DYNAMIC` scenarios to align with quarterly changes for tax years 2026–27 onwards for the following endpoint:
...
- Add a new error `RULE_INCOME_SOURCES_INVALID` for tax years 2023-24 onwards 
...
Remove error `RULE_OUTSIDE_AMENDMENT_WINDOW` and the Gov-Test-Scenario `OUTSIDE_AMENDMENT_WINDOW` for the following endpoints:
